  ...
  return (
    <div className="feature-screen slalom-screen">
      ...
      {!viewingCourse && (
        <>
          {/* ══════════════ OUTER FRAME ══════════════════════════════════ */}
          <div className="slalom-outer-frame">
            ...
            {/* Survey-tab controls (only when schematic is visible) */}
            {tab === 'survey' && (<>
              {/* Live GPS */}
              ...
              {/* ── CONTEXT ACTION BAR (always visible) ────────────────── */}
              {selDef ? (
                /* Buoy selected: show identity + primary + secondary actions */
                <div className="slalom-ctx-bar">
                  {/* Buoy badge + name */}
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0, minWidth:0 }}>
                    <div className="bp-badge" style={{ background: selDef.color, flexShrink:0 }}>{selDef.label}</div>
                    <div style={{ display:'flex', flexDirection:'column', minWidth:0 }}>
                      <span style={{ fontSize:'0.82rem', fontWeight:800, color:'var(--text)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{selDef.name}</span>
                      {selMeas && selStatus && (
                        <span style={{ fontSize:'0.68rem', fontWeight:700, color: STATUS_COLOR[selStatus.status] }}>
                          {STATUS_LABEL[selStatus.status]} · {formatError(selStatus.error)}
                        </span>
                      )}
                    </div>
                  </div>

                  {/* Primary: Save / Update GPS — high-vis yellow/orange */}
                  <button id={`btn-save-buoy-${selectedId}`}
                    className={`btn-primary-action${selMeas ? ' btn-primary-action--update' : ''}`}
                  ...
                  {selMeas && (
                    <button className="btn-ctx-action btn-ctx-action--clear"
                      onClick={() => clearBuoy(selectedId)}>✕</button>
                  )}
                  {selMeas && selectedId !== posRefId && (
                    <button className="btn-ctx-action btn-ctx-action--posref"
                      onClick={() => setPosRefId(selectedId)}>⊙ Pos</button>
                  )}
                  {selMeas && selectedId !== angleRefId && (
                    <button className="btn-ctx-action btn-ctx-action--angref"
                      onClick={() => setAngleRefId(selectedId)}>∠ Ang</button>
                  )}
                  <button className="btn-ctx-action btn-ctx-action--poi"
                    onClick={addPoi} disabled={!hasGps} title="Add POI at current GPS">📍</button>
                  <button className="btn-ctx-action" onClick={() => setSelectedId(null)}>✕ Close</button>
                </div>
              ) : (
                /* No buoy selected: global session actions */
                <div className="slalom-ctx-bar">
                  <button className="btn-ctx-action btn-ctx-action--poi"
                    onClick={addPoi} disabled={!hasGps}>📍 Add POI</button>
                  <button className={`btn-ctx-action${distMode ? ' btn-ctx-action--dist-active' : ''}`}
          ...
          {/* ══════════════ INNER ZONE ═══════════════════════════════════ */}
          <div className="slalom-inner-zone">

            {/* ── SURVEY TAB: schematic fills inner zone ────────────── */}
            {tab === 'survey' && (
              <>
                <SlalomSchematic
                  measured={measured}
                ...
                {/* Buoy detail info (measured coords, theoretical, hint) — in inner zone */}
                {selDef && (
                  <div className="buoy-panel" style={{ borderColor: `${selDef.color}44`, flexShrink: 0 }}>
                    {selMeas && (
                ...
                {selectedPoi !== null && pois[selectedPoi] && (
                  <div className="slalom-poi-card" style={{ flexShrink:0 }}>
                ...
                {!selectedId && selectedPoi === null && (
                  <p className="slalom-tap-hint" style={{ flexShrink:0 }}>Tap a buoy to select it</p>
                )}
              </>
            )}
